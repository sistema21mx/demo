            ...
            <v-form>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="6">
                  <v-dialog
                    ref="dialogdateEvent"
                    v-model="pickModaldateEvent"
                    :return-value.sync="editedItem.dateEvent"
                    persistent
                    width="290px">
                    <template v-slot:activator="{ on }">
                      <v-text-field
                        id="text_dateEvent" ref="text_dateEvent"
                    ...
                    <v-date-picker 
                      v-model="editedItem.dateEvent" 
                      @input="$refs.dialogdateEvent.save(editedItem.dateEvent);pickModaldateEvent = false"
                      >
                    </v-date-picker>
                  </v-dialog>
                </v-col>
  ...
  methods: {
    initialize: async function () {
      ...
      if(await this.editedItem.id===0){
        ...
        this.editedItem.dateEvent = await this.getToday()
      }
    },
    ...
    validationFields: function () {
        ...
        this.editedItem.dateEvent = this.trimField(this.editedItem.dateEvent)
        ...
        if (this.editedItem.dateEvent === null && message === null) {
          message = 'Aviso: Ingresar Fecha'
          document.getElementById('text_dateEvent').focus()
        }
        //
        if(message!==null){
    ...
    updateRow: async function () {
      ...
      const baseURI = await this.urlApi + '/eventupdate'
    ...
    createRow: async function () {
      const methodWay = await this.methodWay
      const baseURI = await this.urlApi + '/eventcreate'
      const dataRespond = await this.typeDataRespond
      this.editedItem.user_id = await this.userData.data.userId
      const formData = await this.editedItem
