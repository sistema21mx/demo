<template>
  <v-container>
      ...
      <v-card>
        ...
        <v-card-text>
          <DiV v-if="imageHeight!==null">{{ timeIn }} {{ timeOut }} {{ datein }} * {{ dateout }}</DiV>
          ...
          <v-container>
            <v-form>
              <v-row>
                ...
                <v-col cols="12" sm="4" md="4">
                  <h4>Entrada</h4>
                  <vue-timepicker v-model="timeIn" format="hh:mm A" hide-clear-button advanced-keyboard :blur-delay="500" id="text_datein" name="text_datein" close-on-complete></vue-timepicker>
                </v-col>
                
                <v-col cols="12" sm="4" md="4">
                  <h4>Salida</h4>
                  <vue-timepicker v-model="timeOut" format="hh:mm A" hide-clear-button advanced-keyboard :blur-delay="500" id="text_dateout" name="text_dateout" close-on-complete></vue-timepicker>
                </v-col>
...
export default {
  ...
  methods: {
    initialize: async function () {
      ...
      if(await this.editedItem.datein===null){
        this.datein = await this.getToday()
        let time = new Date();
        let h = time.getHours()
        let m = time.getMinutes()
        let s = time.getSeconds()
        // this.editedItem.isoIn = this.datein + ' ' + this.leadZero(h,2) + ':' + this.leadZero(m,2) + ':' + this.leadZero(s,2)
        let ampm = h >= 12 ? 'PM' : 'AM'
        h = h % 12
        h = h ? h : 12
      ...
      if(await this.editedItem.datein!==null){
        //
        this.datein = this.editedItem.datein
        ...
        if(this.editedItem.dateout===null){
          //
          this.dateout = await this.getToday()
          let time = new Date();
          let h = time.getHours()
          let m = time.getMinutes()
          let s = time.getSeconds()
          ...
          this.dateout = await this.editedItem.dateout
    ...
    getcondominia_id: async function () {
      ...
      const baseURI = await this.urlApi + '/condominiaids2'
  ...
  watch: {
    ...
    timeIn(val){
      //
      // if((val.hh!=='12' && val.mm!=='00' && val.ss!=='00')){
        let hh = val.hh
        if(val.A==='PM'){
          hh = parseInt(hh) + 12
        }
        this.editedItem.isoIn = this.datein + ' ' + this.leadZero(hh,2) + ':' + this.leadZero(val.mm,2) + ':' + this.leadZero(0,2)
      // }
    },
    timeOut(val){
      ...
      // if((val.hh!=='12' && val.mm!=='00' && val.ss!=='00')){
        ...
        this.editedItem.isoOut = this.dateout + ' ' + this.leadZero(hh,2) + ':' + this.leadZero(val.mm,2) + ':' + this.leadZero(0,2)
        // this.editedItem.isoOut = this.dateout + ' ' + this.leadZero(this.timeOut.hh,2) + ':' + this.leadZero(val.mm,2) + ':' + this.leadZero(0,2)
      // }
    }
  },
  computed: {
