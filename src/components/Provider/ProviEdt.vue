          ...
          <v-container>
            <v-form>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="4">
                  <v-text-field id="text_movil" ref="text_movil" v-on:keyup="keymonitor" :rules="[rules.required]" :disabled="!isEditing"
...
export default {
  ...
  methods: {
    ...
    updateRow: async function () {
      ...
      const baseURI = await this.urlApi + '/providerupdate'
    ...
    createRow: async function () {
      const methodWay = await this.methodWay
      const baseURI = await this.urlApi + '/providercreate'
