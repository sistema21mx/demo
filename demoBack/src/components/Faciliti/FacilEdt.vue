          ...
          <v-container>
            <v-form>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="6">
                  <v-text-field id="text_description" ref="text_description" v-on:keyup="keymonitor" :disabled="!isEditing"
                    v-model="editedItem.description" label="Descripcion"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="6">
                  <v-text-field id="text_regulation" ref="text_regulation" v-on:keyup="keymonitor" :disabled="!isEditing"
                    v-model="editedItem.regulation" label="Reglas"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="6">
                  <v-text-field id="text_cost" ref="text_cost" v-on:keyup="keymonitor" :disabled="!isEditing"
                    v-model="editedItem.cost" label="Costo"></v-text-field>
                </v-col>
...
export default {
  ...
  methods: {
    ...
    updateRow: async function () {
      ...
      const baseURI = await this.urlApi + '/facilitiesupdate'
    ...
    createRow: async function () {
      const methodWay = await this.methodWay
      const baseURI = await this.urlApi + '/facilitiescreate'
      const dataRespond = await this.typeDataRespond
